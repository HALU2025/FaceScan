...
captureBtn.addEventListener('click', () => {
    ...
    mode = "capture"; // 撮影モード
    // 撮影完了後、video, 撮影ボタン, ファイル入力を非表示し、
    // 「この写真で診断」ボタンと「再撮影する」ボタンを表示
    video.style.display = "none";
    captureBtn.style.display = "none";
    fileInput.style.display = "none";
...
fileInput.addEventListener('change', (event) => {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            mode = "file"; // 画像参照モード
            // ファイル選択完了後、video, 撮影ボタン, ファイル入力を非表示し、
            // 「この写真で診断」ボタンと「画像を選びなおす」および「写真を撮影する」ボタンを表示
            video.style.display = "none";
            captureBtn.style.display = "none";
            fileInput.style.display = "none";
            analyzeBtn.style.display = "block";
...
analyzeBtn.addEventListener('click', () => {
    ...
    .then(result => {
        ...
        // 診断結果表示後、シェアボタン群と「もう一回診断する」ボタンを表示
...
// 【再撮影する】ボタン（撮影モード用）
// 再撮影するボタンが押されたら、前の撮影結果を完全にクリアして再撮影状態に戻す
