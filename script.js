...
captureBtn.addEventListener('click', () => {
  ...
  reCaptureBtn.style.display = "inline-block"; // 撮影モードの場合の「再撮影する」ボタン表示
});

// 【ファイル選択処理】 画像参照でファイルが選ばれた場合
fileInput.addEventListener('change', (event) => {
  ...
  if (file) {
    ...
    reader.onload = function(e) {
      ...
      fileInput.style.display = "none";
      analyzeBtn.style.display = "block";       // 「この写真で診断」ボタンを表示
      selectAgainBtn.style.display = "inline-block"; // 画像参照の場合の「画像を選びなおす」ボタン表示
      takePhotoBtn.style.display = "inline-block";     // 「写真を撮影する」ボタン表示
    };
...
analyzeBtn.addEventListener('click', () => {
  ...
  .then(result => {
    ...
    // 診断結果が出たら、シェアボタンと「もう一回診断する」ボタンを表示
...
// 【診断結果の画像化機能】 診断結果のテキストを Canvas に描画し、画像データに変換してプレビューに表示
